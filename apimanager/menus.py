...
log = logging.getLogger("red.drapercogs.apimanager.menus")
...
class HelpSource(_dpy_menus.ListPageSource):
...
class LeaderboardSource(_dpy_menus.ListPageSource):
    ...
    async def format_page(
        ...
        for pos, acc in enumerate(entries, start=position):
            try:
                user = guild.get_member(int(acc[1])) or bot.get_user(int(acc[1]))
