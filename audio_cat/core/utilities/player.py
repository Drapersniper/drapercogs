...
class PlayerUtilities(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def update_bot_presence(
        self, track: Optional[str], playing_servers: int
    ) -> None:
    ...
    async def _skip_action(
        ...
        if skip_to_track is not None and skip_to_track != 1:
            ...
            await self.send_embed_msg(ctx, embed=embed)
        self.bot.dispatch(
            "red_audio_skip_track", player.channel.guild, player.current, ctx.author
        )
        await player.play()
        ...
        if not first_track_only and len(tracks) > 1:
            ...
            try:
                ...
                if not await self.is_query_allowed(
                    ...
                    if self.is_track_length_allowed(
    ...
    async def maybe_move_player(self, ctx: commands.Context) -> bool:
        ...
        try:
            in_channel = sum(
                not m.bot
                for m in ctx.guild.get_member(self.bot.user.id).voice.channel.members
            )
        except AttributeError:
            return False
