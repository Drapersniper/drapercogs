...
class EqualizerUtilities(MixinMeta, metaclass=CompositeMetaClass):
    async def _apply_gain(self, guild_id: int, band: int, gain: float) -> None:
        ...
        try:
            await self.bot.wavelink.get_player(guild_id).node.send({**const})
        except (KeyError, IndexError, wavelink.errors.ZeroConnectedNodes):
            pass
    ...
    async def _eq_check(self, ctx: commands.Context, player: Player) -> None:
