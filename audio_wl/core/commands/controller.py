...
from ...wavelink_overrides import Player
...
class PlayerControllerCommands(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def command_summon(self, ctx: commands.Context):
        ...
        try:
            ...
            if not self.player_ready(ctx):
                ...
                if ctx.guild.me.voice and ctx.author.voice.channel == player.channel:
                    ctx.command.reset_cooldown(ctx)
                    return
        ...
        except (IndexError, wavelink.errors.ZeroConnectedNodes):
            ctx.command.reset_cooldown(ctx)
            return await self.send_embed_msg(
                ctx,
                title=_("Unable To Join Voice Channel"),
    ...
    async def command_volume(self, ctx: commands.Context, vol: int = None):
        ...
        if vol > 150:
            ...
            if self.player_ready(ctx):
                await self.bot.wavelink.get_player(
                    ctx.guild.id, cls=Player, guild=ctx.guild, channel=ctx.author.voice.channel
                ).set_volume(vol)
        else:
            await self.config.guild(ctx.guild).volume.set(vol)
    ...
    async def command_remove(self, ctx: commands.Context, index_or_url: Union[int, str]):
        ...
        if isinstance(index_or_url, int):
            if index_or_url > player.queue.qsize() or index_or_url < 1:
                return await self.send_embed_msg(
                    ctx,
                    title=_("Unable To Modify Queue"),
                    description=_(
            ...
            index_or_url -= 1
            removed = player.queue._queue.pop(index_or_url)
            ...
            async for track in AsyncIter(player.queue):
                if track.uri != index_or_url:
                    ...
                    removed_tracks += 1
            player.queue = asyncio.Queue()
            player.queue._queue = collections.deque(clean_tracks)
            if removed_tracks == 0:
                await self.send_embed_msg(
                    ctx,
                    title=_("Unable To Modify Queue"),
    ...
    async def command_bump(self, ctx: commands.Context, index: int):
        ...
        if index > player.queue.qsize() or index < 1:
            return await self.send_embed_msg(
                ctx,
                title=_("Unable To Bump Track"),
        ...
        bump_index = index - 1
        bump_song = await player.queue.pop(bump_index)
        bump_song.extras["bumped"] = True
        player.queue._queue.insert(0, bump_song)
        description = await self.get_track_description(bump_song, self.local_folder_current_path)
        await self.send_embed_msg(
