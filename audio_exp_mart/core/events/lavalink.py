                ...
                try:
                    ...
                    if notify_channel:
                        ...
                        await self.send_embed_msg(
                            notify_channel, title=_("Couldn't get a valid track."), error=True
                        )
                    return
        if event_type == lavalink.LavalinkEvents.TRACK_START and notify:
            ...
            if notify_channel:
                ...
                notify_message = await self.send_embed_msg(
                    ...
                    author={
                        "name": _("Now Playing"),
                        "url": "https://cdn.discordapp.com/emojis/528753381418860595.gif",
                    },
                )
        ...
        if event_type == lavalink.LavalinkEvents.QUEUE_END:
            if not autoplay:
                ...
                if notify_channel and notify:
                    notify_channel = self.bot.get_channel(notify_channel)
                    await self.send_embed_msg(
                        notify_channel, title=_("Queue ended."), success=True
                    )
                if disconnect:
                ...
                if early_exit:
                    embed = discord.Embed(
                        ...
                        description=_(
                            ...
                            "[join the support server](https://discord.gg/R6puN8Z) "
                            "to follow the situation."
                        ),
                    )
                    await self.send_embed_msg(message_channel, embed=embed, error=True)
                    return
                else:
