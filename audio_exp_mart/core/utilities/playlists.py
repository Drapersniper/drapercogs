...
class PlaylistUtilities(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def _load_v3_playlist(
        ...
        embed1 = discord.Embed(
            title=_("<a:loading:523897462445047808> Please wait, adding tracks...")
        )
        ...
        embed2 = discord.Embed(
            colour=await ctx.embed_colour(),
            title=_("<a:loading:523897462445047808> Loading track {num}/{total}...").format(
    ...
    async def _load_v2_playlist(
        ...
        notifier = Notifier(
            ctx,
            playlist_msg,
            {"playlist": _("<a:loading:523897462445047808> Loading track {num}/{total}...")},
        )
