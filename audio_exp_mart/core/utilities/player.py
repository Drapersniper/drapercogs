...
class PlayerUtilities(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def _get_spotify_tracks(
        ...
        if ctx.invoked_with in ["play", "genre", "pl"]:
            enqueue_tracks = True
        else:
    ...
    async def _enqueue_tracks(
        ...
        if not first_track_only and len(tracks) > 1:
            ...
            embed = discord.Embed(
                ...
                title=_("\N{MULTIPLE MUSICAL NOTES} Playlist Enqueued"),
            )
            embed.set_footer(
    ...
    async def fetch_spotify_playlist(
        ...
        try:
            embed1 = discord.Embed(
                title=_("<a:loading:523897462445047808> Please wait, finding tracks...")
            )
            embed1.set_author(
                name=" ", icon_url="https://cdn.discordapp.com/emojis/523897462445047808.gif"
            )
            ...
            notifier = Notifier(
                ...
                playlist_msg,
                {
                    "spotify": _("<a:loading:523897462445047808> Getting track {num}/{total}..."),
                    "youtube": _("<a:loading:523897462445047808> Matching track {num}/{total}..."),
                    "lavalink": _("<a:loading:523897462445047808> Loading track {num}/{total}..."),
