...
import logging
import re
from typing import Any, Final, MutableMapping, Optional, Union, cast
...
_TIME_RE = re.compile(_TIME_RE_STRING, re.I)
...
class MiscellaneousUtilities(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def send_embed_msg(self, ctx: commands.Context, **kwargs) -> discord.Message:
        ...
        if (
            (error or success)
            and contents.get("title")
            and contents.get("title") is not EmptyEmbed
        ):
            contents["title"] = (
                EmojiMapper.get("CROSS", "")
                if error
                else EmojiMapper.get("SUCCESS", "") + " " + contents["title"]
            )
        if (
            (error or success)
            and contents.get("description")
            and contents.get("description") is not EmptyEmbed
        ):
            contents["description"] = (
                EmojiMapper.get("CROSS", "")
                if error
                else EmojiMapper.get("SUCCESS", "") + " " + contents["description"]
            )
        ...
        footer = contents.get("footer", {}).get("text") or footer
        if footer:
            if error or success:
                text = (
                    EmojiMapper.get("CROSS", "")
                    if error
                    else EmojiMapper.get("SUCCESS", "") + " " + footer
                )
            else:
                text = footer
    ...
    def parse_timedelta(self, argument: str,) -> Optional[datetime.timedelta]:
        """
        ...
        """
        matches = _TIME_RE.match(argument)
        if matches:
