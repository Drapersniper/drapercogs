...
class FormattingUtilities(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def _search_button_action(
        ...
        if not self._player_check(ctx):
            if self.lavalink_connection_aborted:
                ...
                return await self.send_embed_msg(
                    ctx, title=msg, description=description, error=True
                )
            try:
                ...
                return await self.send_embed_msg(
                    ctx, title=_("Connect to a voice channel first."), error=True
                )
            except IndexError:
                return await self.send_embed_msg(
                    ctx,
                    title=_("Connection to Lavalink has not yet been established."),
                    error=True,
                )
        player = lavalink.get_player(ctx.guild.id)
        ...
        if not await self.is_query_allowed(
            ...
            return await self.send_embed_msg(
                ctx, title=_("This track is not allowed in this server."), error=True
            )
            ...
            if self.is_track_too_long(search_choice.length, guild_data["maxlength"]):
                ...
                return await self.send_embed_msg(
                    ctx, title=_("Track exceeds maximum length."), error=True
                )
        else:
            search_choice.extras.update(
                {
