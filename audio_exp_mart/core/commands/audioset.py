...
from redbot.core.utils.menus import CUSTOM_DEFAULT_CONTROLS, menu, start_adding_reactions
...
class AudioSetCommands(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def command_audioset_perms_global_whitelist_add(
        ...
        if exists:
            return await self.send_embed_msg(
                ctx, title=_("Keyword already in the whitelist."), error=True,
            )
        else:
            return await self.send_embed_msg(
                ctx,
    ...
    async def command_audioset_perms_global_whitelist_list(self, ctx: commands.Context):
        ...
        if not whitelist:
            return await self.send_embed_msg(
                ctx, title=_("Nothing in the whitelist."), error=True,
            )
        whitelist.sort()
        text = ""
        total = len(whitelist)
        ...
        if len(pages) == 1:
            return await self.send_embed_msg(ctx, embed=pages[0])
    ...
    async def command_audioset_perms_global_blacklist_add(
        ...
        if exists:
            return await self.send_embed_msg(
                ctx, title=_("Keyword already in the blacklist."), error=True,
            )
        else:
            return await self.send_embed_msg(
                ctx,
    ...
    async def command_audioset_perms_global_blacklist_list(self, ctx: commands.Context):
        ...
        if not blacklist:
            return await self.send_embed_msg(
                ctx, title=_("Nothing in the blacklist."), error=True,
            )
        blacklist.sort()
        text = ""
        total = len(blacklist)
    ...
    async def command_audioset_perms_global_blacklist_delete(
        ...
        if not exists:
            return await self.send_embed_msg(
                ctx, title=_("Keyword is not in the blacklist."), error=True
            )
        else:
            return await self.send_embed_msg(
                ctx,
    ...
    async def command_audioset_perms_whitelist_list(self, ctx: commands.Context):
        ...
        if not whitelist:
            return await self.send_embed_msg(ctx, title=_("Nothing in the whitelist."), error=True)
        whitelist.sort()
        text = ""
        total = len(whitelist)
        pages = []
