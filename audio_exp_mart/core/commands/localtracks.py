...
from redbot.core.utils.menus import CUSTOM_DEFAULT_CONTROLS, close_menu, menu, next_page, prev_page
...
class LocalTrackCommands(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def command_local_play(self, ctx: commands.Context):
        ...
        if not localtracks_folders:
            return await self.send_embed_msg(ctx, title=_("No album folders found."), error=True)
    ...
    async def command_local_search(self, ctx: commands.Context, *, search_words):
        ...
        if not search_list:
            return await self.send_embed_msg(ctx, title=_("No matches."), error=True)
