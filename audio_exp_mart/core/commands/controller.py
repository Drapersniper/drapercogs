...
class PlayerControllerCommands(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def command_disconnect(self, ctx: commands.Context):
        ...
        if not self._player_check(ctx):
            return await self.send_embed_msg(ctx, title=_("Nothing playing."), error=True)
        else:
            ...
            await self.send_embed_msg(ctx, title=_("{} Bye!").format(self.get_wave_emoji(ctx)))
    ...
    @commands.group(name="now", aliases=["n"], autohelp=False)
    @commands.guild_only()
    ...
    async def command_now(self, ctx: commands.Context):
        ...
        if ctx.invoked_subcommand is None:
            ...
            text += (
                _("Auto-Play")
                + ": "
                + ("\N{WHITE HEAVY CHECK MARK}" if autoplay else self.get_cross_emoji(ctx))
            )
            text += (
                (" | " if text else "")
                + _("Shuffle")
                + ": "
                + ("\N{WHITE HEAVY CHECK MARK}" if shuffle else self.get_cross_emoji(ctx))
            )
            text += (
                (" | " if text else "")
                + _("Repeat")
                + ": "
                + ("\N{WHITE HEAVY CHECK MARK}" if repeat else self.get_cross_emoji(ctx))
            )
            ...
            if react == "prev":
                ...
                await ctx.invoke(self.command_volume, vol=vol + 10)
            elif react == "vol_down":
                await self._clear_react(message, emoji)
                vol = await self.config.guild(ctx.guild).volume()
                await ctx.invoke(self.command_volume, vol=vol - 10)
    ...
    @command_now.command(name="persist")
    @commands.guild_only()
    @commands.bot_has_permissions(embed_links=True, add_reactions=True)
    async def command_now_persist(self, ctx: commands.Context):
        """Now playing."""
        if not self._player_check(ctx):
            return await self.send_embed_msg(ctx, title=_("Nothing playing."), error=True)
        expected = ("⏮", "⏹", "⏯", "⏭", "➕", "➖", "ℹ", "🔄")
        ...
        player.store("np_message", message)
        self.now_message_cache[ctx.guild.id] = (message, ctx, player)
        ...
        await asyncio.create_task(
            self.make_now_task(ctx, player, message, expected, emoji, timeout=3600.0)
        )

    @commands.command(name="pause", aliases=["resume", "res", "pa"])
    @commands.guild_only()
    @commands.bot_has_permissions(embed_links=True)
    ...
    async def command_pause(self, ctx: commands.Context):
        ...
        if player.current and not player.paused:
            await player.pause()
            return await self.send_embed_msg(
                ctx, title=_("Track Paused"), description=description, success=True
            )
        if player.current and player.paused:
            await player.pause(False)
            return await self.send_embed_msg(
                ctx, title=_("Track Resumed"), description=description, success=True
            )

        await self.send_embed_msg(ctx, title=_("Nothing playing."), error=True)
    ...
    @commands.group(name="shuffle", autohelp=False, aliases=["sh"])
    @commands.guild_only()
    @commands.bot_has_permissions(embed_links=True)
    @commands.max_concurrency(1, per=commands.BucketType.guild, wait=True)
    ...
    @commands.command(name="summon", aliases=["connect", "join"])
    @commands.guild_only()
    ...
    async def command_remove(self, ctx: commands.Context, index_or_url: Union[int, str]):
        ...
        can_skip = await self._can_instaskip(ctx, ctx.author)
        if not player.queue:
            return await self.send_embed_msg(ctx, title=_("Nothing queued."), error=True)
        if dj_enabled and not can_skip:
            return await self.send_embed_msg(
                ctx,
